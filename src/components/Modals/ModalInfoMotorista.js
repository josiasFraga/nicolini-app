...
import ModalMotorista from '@components/Modals/ModalMotorista';
  ...
  setModalVisible = (visible) => {
    ...
    ImagePicker.showImagePicker(options, response => {
      ...
      CompressImage.createCompressedImage(
        ...
        this.setState({
          motorista_foto: source.uri,
          //choosingImage: false,
        });
        this.props.sendUserPhoto(source, this.props.item.Usuario.id);
        })
        .catch(err => {
        console.log(err);
  ...
  render() {
    let sectionsList = 
      [{
        ...
        data: [
          {title: this.props.item.Usuario['telefone'], iconName: 'phone', iconType: "antdesign"},
        ]
      }];

      if ( this.props.item.Veiculos.length > 0 ) {
          sectionsList.push(
            {
              title: 'Veiculos',
              data: this.props.item.Veiculos
            }
          );
      }
        ...
        <View
          style={{
            ...
            <ScrollView style={styles.scrollView}>
                ...
                <Header titulo="Motorista" backButton righElement='edit' newSceneType="modal" functionOpenNewScene={this.setModalVisible} />

                <View>
                    <ImageBackground 
                        source={{ uri: this.state.motorista_foto == '' ? this.props.item.Usuario.img : this.state.motorista_foto }} 
                        style={{width: '100%', height: 220}}
                        resizeMode='contain' 
                    >
                        <View style={{position: 'absolute', bottom: 10, right:10}}>
                            <Text style={styles.subtitle}>{this.props.item.Usuario.nome}</Text>
                            <Text style={styles.subtitle}>{this.props.item.Usuario.telefone}</Text>
                    ...
                    renderItem={({ item }) => {
                      return (
                        <ListItem
                          ...
                          subtitle={() => {
                              if (typeof(item.bottomContent) != 'undefined') {
                                return (
                                  <View style={GlobalStyle.row}>
                                    <Text>{item.bottomContent}</Text>
                                  </View>
                              )}                            
                          }}
                          rightElement={() => {
...
const mapDispatchToProps = dispatch => ({
  sendUserPhoto(photo, usuario_id) {
    dispatch({
      type: 'SEND_USER_PHOTO',
      payload: {
        photo: photo,
